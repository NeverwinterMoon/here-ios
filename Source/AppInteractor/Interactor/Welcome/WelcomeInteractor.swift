...
public final class WelcomeInteractor {
    ...
    public func sendEmail(email: String, username: String, password: String) -> Single<Void> {
        ...
        return API.User.Create(email: email, username: username, password: password).asSingle().flatMap { user -> Single<Void> in
    ...
    private func storeUserToRealm(user: User) -> Single<Void> {
